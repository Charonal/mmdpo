import json
import re
from utils import extract_output,build_batch_datas,map_output,batch_formula_prompt
...
def main():
    file = '/data/lyz/math_dpo/datas/gsm8k/gsm8k_mask.json'
    model = 'qwen'
    output_file = '/data/lyz/math_dpo/datas/gsm8k_formular.json'
    if model == 'qwen':
        model_path = '/data/lyz/qwen/Qwen2.5-7B-Instruct'
    elif model =='step_dpo':
    ...
    batch_size = 20
    gen_nums = 5
    llm = LLM(model=model_path, trust_remote_code=True, tensor_parallel_size=2,tokenizer_mode="auto", dtype="auto",gpu_memory_utilization=0.5)
    sampling_params = SamplingParams(temperature=0.7, top_p=0.9,max_tokens=512)
    datas_final =[]
    batch_datas = build_batch_datas(datas,batch_size)
    for data in tqdm(batch_datas):
        batch = batch_formula_prompt(data,gen_nums)
